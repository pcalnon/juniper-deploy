...
| `test_compose_data_egress.py` | — | `data-egress`'s definition is pinned exactly and it is juniper-data-only; every service attaches to networks the file declares; no service uses `extends`; `backend` / `data` stay internal; juniper-data publishes no port |
| `test_helm_networkpolicy_data_egress.py` | — | Evaluates every NetworkPolicy selecting a pod, as Kubernetes unions them: the data pod's effective egress is exactly DNS plus TCP 443 to public IPv4; every other Juniper pod is governed with no `ipBlock` or peerless non-DNS egress; the data pod mounts no service-account token (skips without `helm`) |
