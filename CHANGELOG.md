...
- **`scripts/doctor.sh` — service list derived from the compose render, not hardcoded** (drift spotted while grounding the image-provenance preflight). Doctor's hardcoded 4-entry `SERVICES` array had drifted: `juniper-recurrence` and the `juniper-cascor-demo` / `juniper-canopy-demo` / `juniper-canopy-dev` variants were never checked. Worse, every built service is profile-gated while the `docker compose ps -q <service>` lookup passed **no** `--profile` flags — compose could not resolve the service name, so the "prefer the running container's image" branch never fired and doctor silently always inspected the built `:latest` tag instead. Now doctor derives `(service, image, build-context)` rows from `docker compose config --format json` (all profiles by default; same mechanism as the preflight family), resolves each build context to its **enclosing** git repo (nested contexts like `../juniper-recurrence/juniper-recurrence` work), and passes the same profile args to the `ps` lookup so the running container's image really is preferred when up. Classification (FRESH / STALE / UNKNOWN / DIRTY, prefix compare, `-dirty` precedence) and exit semantics are unchanged; coverage went from 4 services to all 8. Offline `--config-json` + `--image-provenance-map` seams added for CI. New hermetic gate `tests/test_doctor_provenance_derivation.py` (13 tests: per-service rows incl. shared images + nested contexts, classification parity, skip rules, exit codes, and a shipped-compose coverage-domain drift gate); `tests/test_makefile_doctor.py`'s hardcoded-list lint replaced with a derivation-mechanism lint that also forbids reintroducing a `SERVICES=(` array.
