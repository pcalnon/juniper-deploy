...
- **Build provenance (juniper-ml [#412](https://github.com/pcalnon/juniper-ml/pull/412) design)**: every juniper image is now stampable with its source git SHA + build date at `make build` time. `docker-compose.yml` declares `build.args` (`GIT_SHA` / `BUILD_DATE` / `APP_VERSION`) on all **7** juniper build blocks (data, cascor, cascor-demo, worker, canopy, canopy-demo, canopy-dev), each referencing its **own** repo's per-repo var (`GIT_SHA_DATA` / `_CASCOR` / `_CANOPY` / `_WORKER`); the demo/dev variants reuse their parent's var since they share its build context. The `Makefile` `PROVENANCE_ENV` computes each repo's short git SHA + `pyproject` version (from `../<repo>`) plus a single shared ISO-8601 `BUILD_DATE`, prefixed to `build` / `build-no-cache` — a single global `GIT_SHA` would mislabel a multi-service build, hence per-repo vars. New `tests/test_compose_build_provenance_wired.py` (3 tests) asserts the wiring on all 7 blocks, guards against a global `GIT_SHA`, and pins the compose↔Makefile var contract. The 3 service Dockerfiles (data/cascor/canopy) ignore these build-args (harmless warning) until their provenance PRs land; the worker Dockerfile already consumes them ([cascor-worker #103](https://github.com/pcalnon/juniper-cascor-worker/pull/103)). The `make doctor` drift checker + `scripts/health_check.sh` column follow in a second deploy PR.
